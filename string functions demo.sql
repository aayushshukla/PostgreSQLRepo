...
--upper()
select upper(title) Title from employees;

--concat(str1,str2,str3)
select concat('aayush','shukla',10) username;

select concat (title_of_courtesy,'.',first_name,' ',last_name) FullName from employees;
...
select trim(Both '*w' from '***www.google.com*****') as webname;
...
select substring(first_name,1,3) as subname,first_name from employees;
...
select position('x' in 'good evening') as Ofound;
...
--MD5()
select MD5(first_name) as encryptedname from employees;
